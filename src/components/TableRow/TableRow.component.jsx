...
const TableRow = ({ item, setOrders, setEditModalOpen, setEditOrder }) => {
  ...
  return (
    <tr>
      <td className="text-base border text-center border-blue-900 ">
        {item.clientId}
      </td>
      <td className="text-base border text-center border-blue-900 ">
        {item.orderStatus}
      ...
      <td className="text-base border text-center border-blue-900 ">
        $ {item.deliveryDiscount ? item.deliveryDiscount : 0}
      </td>
      <td className="text-base border text-center border-blue-900 ">
        {item.guideNumber}
      </td>
      <td className="text-base border text-center border-blue-900 ">
        {transport[item.transportType].place}
      </td>
      <td className="text-base border text-center border-blue-900 ">
        {transport[item.transportType].vehicle}
      </td>
      <td className="text-base  text-center text-white ">
        <div className="flex flex-row items-center gap-2 h-full pl-2">
          <div
            onClick={handleEdit}
            className="bg-blue-900 rounded-sm p-3 hover:cursor-pointer h-full w-full"
          >
            <AiOutlineEdit />
          </div>
          <div
            onClick={handleDelete}
            className="bg-red-500 rounded-sm p-3 hover:cursor-pointer h-full w-full"
          >
            <HiOutlineTrash />
          </div>
        </div>
