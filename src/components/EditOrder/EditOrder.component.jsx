...
import {
  ...
  createOrder,
  editOrder,
} from "../../util/orders.utils";
import {
  transportTypes,
  statusTypes,
  availableClients,
} from "../../util/dropdowns.utils";
...
const EditOrder = ({ orderToEdit, setOrders, onClose }) => {
  ...
  const validate = Yup.object({
    ...
    transportType: Yup.mixed()
      .oneOf(["sea", "land"], 'MUST be either "sea" or "land"')
      .required("Required"),
    ...
    registerDate: Yup.string()
      .matches(
        ...
        "A valid yyyy-mm-dd format is required"
      )
      .required("Required"),
    ...
    guideNumber: Yup.string()
      .matches(/^[0-9]{10}$/, "Must contain 10 numbers")
      .required("Required"),
    ...
    fleetNumber:
      isTransportType === "sea"
        ? Yup.string()
            ...
            .required("Required")
        : Yup.string()
            .matches(/^[a-zA-Z]{3}[0-9]{3}$/, "3 Letters,3 numbers sequece")
            .required("Required"),
  });
        ...
        <div className="fixed h-[70vh] w-[40vw] top-[50%] left-[50%] translate-x-[-50%] translate-y-[-50%]">
          <div className="bg-white h-full w-full overflow-x-scroll ">
            <h1 className="text-center p-6 pb-0">
              {newOrder ? "Create" : "Edit"} Order
            </h1>
            <Form>
              <div className="grid grid-cols-2 gap-5 p-8">
                <div>
                  <TextFieldSelect
                    value={isTransportType}
                    label="Transport Type"
                    name="transportType"
                    onInput={(e) => setIsTransportType(e.target.value)}
                  ...
                  <TextFieldSelect
                    ...
                    label="Order Status"
                    name="orderStatus"
                    onInput={(e) => setStatusSelect(e.target.value)}
                ...
                <div>
                  {isAdmin() ? (
                    <TextFieldSelect
                      ...
                      label="Client"
                      name="clientId"
                      onInput={(e) => setClientSelect(e.target.value)}
                  ...
                  <TextFieldEditCreate
                    label={`${isTransportType === "sea" ? "Ship ID" : "Truck"}`}
                    name="fleetNumber"
                    type="text"
                  />
                  <TextFieldEditCreate
                    label={`${
                      isTransportType === "sea" ? "Port Delivery" : "Warehouse"
                    }`}
                    name="portDelivery"
                    type="text"
                  />
              ...
              <div className="flex flex-row justify-center">
                <CustomButton
                  type="submit"
                  name={`${newOrder ? "Create Order" : "Edit Order"}`}
                >
                  Submit
                </CustomButton>
              </div>
