...
const Truck = () => {
    ...
    return (
        <div className='h-[87vh] w-[80vw] overflow-x-scroll max-h-[87vh]'>
            <div onClick={handleNewtrackClick} className='text-xl text-blue-900 text-center flex flex-row items-center gap-2 cursor-pointer w-fit p-2 hover:bg-blue-500 hover:text-white rounded-sm'>
                <h1>Add a New Truck</h1>
                <AiOutlinePlus />
            </div>
            {editTruckOpen && <Modal open={editTruckOpen} onClose={() => setEditTruckOpen(false)}><EditTruck setTrucks={setTrucks} truckToEdit={editTrucks} onClose={() => { setEditTruckOpen(false); setEditTrucks('') }} /></Modal>}
            {trucks.length === 0 ? <div className='text-center'>No Trucks to Display</div> : <>

                <table className="w-[110%] table-auto font-thin border-blue-900 text-left m-3 ">
                    <thead>
                        <tr className='text-xs font-bold w-auto'>
                            <td className='w-auto px-2 text-left border border-blue-900 min-w-fit'>Truck ID</td>
                    ...
                    <tbody>
                        {trucks.map((item) => {
                            return (
                                ...
                                <TrucksRow key={item._id} item={item} setTrucks={setTrucks} setEditModalOpen={setEditTruckOpen} setEditTruck={setEditTrucks} />
                            )
                        })}
